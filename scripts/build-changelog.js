...
// 개발자용 Log는 전체 버전 포함, 로컬 개발용으로만 dev/ 폴더에 생성한다.
// outputDirectory가 "."라 .vercelignore는 "빌드 중에 새로 생성되는 파일"에는
// 적용되지 않으므로(업로드 시점 필터일 뿐, 빌드 산출물은 그대로 배포됨),
// Vercel 빌드 환경(VERCEL=1)에서는 아예 파일을 생성하지 않아 배포 노출을 원천 차단한다.
if (process.env.VERCEL) {
  console.log('ℹ Vercel 빌드 환경 감지: changelog_dev.json은 배포 노출 방지를 위해 생성하지 않음');
} else {
  const devData = rows
  ...
  console.log(`✓ dev/changelog_dev.json 생성 완료: ${devData.length}개 버전 (배포 제외, 로컬 전용)`);
}
